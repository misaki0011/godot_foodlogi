...
| UI-01 | Map view overlays | Done | P0 | TERR-01, ROUTE-01 | Sources, settlements, routes (tinted by level), storage, hubs, congestion markers, hub/settlement hover info, a per-source speech bubble (food color + daily supply, always on), and a per-settlement speech-bubble icon (food color + shortfall number) for any food still short of demand on the last simulated day are all visible together on the map. Bubbles are baked once to a texture and shown on a billboard so their text never sorts behind the bubble shape. Animated food movement is not (see LOOP-03). | §10.1 |
