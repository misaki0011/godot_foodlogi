...
| UI-03 | Mobile zoom/pan controls | Done | P1 | TERR-01 | A fixed top-left panel provides +/− zoom buttons and a 4-direction pan pad for exploring the map by mouse or touch, continuous while held and clamped to a small margin past the map edge. | §10.7 |
...
The HTML places Farm at (3,4) and Bakery at (3,9), only 2-3 tiles (Chebyshev distance) from Garden at (2,6). With TERR-03's always-on supply speech bubbles, sources that close together crowded each other's bubbles. This port moves Farm to (3,1) and Bakery to (3,11) -- Garden, Dairy, and Harbor keep their original HTML coordinates -- so every source is at least 4 tiles from every other source. Supply amounts, food mappings, and all other node positions are unchanged.
