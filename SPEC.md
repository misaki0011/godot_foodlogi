...
64. **A placed tile presses up out of the ground.** Building was the one action on the map with no feedback of its own: a tile simply existed on the next frame, which reads as a redraw rather than as a thing the player did. A newly placed tile now starts squashed and springs to full size with a small overshoot, over **0.26s**.
    **Squashed, not uniformly small.** A road slab is 2.0 world units across and 0.22 tall, so a uniform scale-from-small is dominated by the *footprint* growing -- which reads as the tile sliding in from off-map. Starting at `(0.85, 0.3, 0.85)` makes **height** the thing that changes, and height arriving is what placement looks like. The geometry pays for the rest: a road block's own origin sits at its mid-height, so a flattened slab has half its shrink below the grass cap it stands on and the terrain plinth occludes it -- the tile emerges from the ground with no pivot node to maintain. Markers are authored origin-at-base (`hub_marker.tscn`), so they grow up out of the road instead.
    **Scale is the verb item 59 reserved for POP**, a new order opening, and that is not the collision it looks like: that vocabulary governs the billboards floating *above* nodes, this is the ground plane, and the two never flourish for the same reason at the same moment. They are deliberately not the same motion either -- a column springs from 0.18 over 0.50s, a tile from a squash over half that, because a tile is a smaller and far more frequent event.
    **A drag staggers, and that is the part that makes it feel built.** A route drag places its whole run at once; popping the run in unison is a flash, so each tile is delayed **0.045s** behind the one before it in the order the player drew, and the road lays itself end to end. A ten-tile drag finishes in 0.71s. The stagger is clamped to a **0.5s budget** for the whole run, tightening rather than extending when a sweep is long, so a gesture never becomes something the player waits out.
    **Only the part that arrived pops.** A cell draws as up to two things -- the road surface, and whatever stands on it -- and building a hub, storage or a bridge adds a structure to a road that was **already there** (§4.1, §5). Popping that road would announce a tile the player never placed, so the pending set records a `TilePart` and the render pops that alone. Upgrades get nothing: a tile that changes level did not arrive, and matching amplitudes would blur "new" against "improved".
    **Bulldozing is the inverse**, squashing flat to `(0.7, 0.05, 0.7)` over 0.2s with no overshoot -- easing out would read as a very fast pop played backwards rather than as the tile being crushed away. The removed visual cannot be animated in place, since `_clear_cell` mutates the grid and the render that follows clears `GridVisuals` wholesale, so a throwaway copy goes on the `Effects` layer that item 60 established for exactly this, and frees itself when its tween ends. Which part squashes is the placement rule read backwards: clearing a hub, storage or bridge leaves the road it stood on, so only the structure goes.
    **The breakable part is the bookkeeping, again.** `_render_grid` rebuilds every tile on each hover, day and build, so a pending pop that is never cleared leaves the whole network flexing whenever the pointer crosses a node -- item 57's failure mode, on a much larger surface. `_pending_tile_pops` is therefore cleared whole by the render that consumes it, beside `_pending_pops`. Each pop's tween is bound to **the tile** rather than to `Main`, so a render mid-pop takes the tween with the node it frees instead of animating a freed target; the tile the new render draws is simply at rest, which is the right answer for a pop that was cut short. `verify_main._test_tile_placement_pops_once` asserts a drag arms one road pop per tile in draw order, that a committed drag leaves every tile of its run mid-pop with nothing pending, that a second render pops nothing, that a hub pops its **marker and not its road**, and that a bulldoze leaves exactly one ghost on `Effects` which a re-render does not destroy. See §10.1.

65. **The pop becomes a punch, and every construction gets it.** Item 64 shipped one squash easing out to full size, which arrives but does not *land*. A construction now moves through **three poses**: splatted flat and spread wide, past full size stretched tall and narrow, then settling with a small dip.
    **The two axes move in opposition, and that is the whole trick.** A uniform scale-from-small is dominated by the footprint growing and reads as the tile sliding in from off-map; XZ going wide-then-pinched while Y goes flat-then-tall is squash and stretch, and it reads as impact at any silhouette. That matters because the two things being animated have **opposite proportions** -- a road slab is 2.0 world units across and 0.22 tall, so its visible axis is the footprint, while a hub or storage building is tall and its visible axis is height. One motion covers both because each part carries its own amounts: a road splats to `(1.12, 0.05, 1.12)` and stretches to `(0.93, 1.28, 0.93)`, a structure splats harder to `(1.25, 0.12, 1.25)` and stretches to `(0.86, 1.24, 0.86)`. A road stays nearer 1.0 on XZ because at rest its tile is **flush with its neighbours**, so the splat overlaps them -- brief, at a phase the stagger keeps out of sync with the tiles either side, and squash is exactly what it reads as. A structure has clear space around it and can splat harder.
    **Fast in, slower out.** The attack is what makes it a punch: `TRANS_QUINT` front-loads almost all of the travel into the first few frames of an 0.11s rise, against a 0.20s settle. The settle uses `TRANS_BACK`, which undershoots past 1.0 before returning -- that dip is the secondary squash, and it comes from the easing rather than a fourth keyframe. The stagger tightens to **0.035s** on a 0.4s budget to match, so a ten-tile drag still finishes in 0.66s despite the longer motion.
    **Removal is the punch backwards but not symmetric**: a 0.07s anticipation stretch, then a 0.16s crush that **spreads** to `(1.3, 0.02, 1.3)` rather than shrinking. A uniform shrink to nothing reads as the tile flying away from the camera, which is a different event from being demolished.
    **And it now covers everything the player builds.** Item 64 left two out. A **route upgrade** swaps the block outright -- Dirt, Paved and Main are three different meshes -- so what the player is left looking at genuinely is new, and it lands; a swept upgrade staggers along the sweep like a drag, since the player drew a line and the road should resurface along it. An **expanded source** (DEV-03) is the one construction that is not a grid cell: its markers live in `NodeMarkers`, which `_render_grid` never rebuilds, so they are popped **directly** rather than through `_pending_tile_pops` -- there is no rebuild to survive and so nothing to arm or clear. A source occupies several cells and gets one marker each (DEV-02); they land **together**, because they are one building and a stagger would say they were two. `verify_main._test_tile_placement_pops_once` now also asserts that a landing structure starts flat *and spread* (a splat that shrank on every axis would read as falling away from the camera), that an upgraded tile pops exactly once, and that a source's markers land and **survive a later render**. It restores the treasury and un-expands the source it borrows, since the DEV-03 check further down expects a map nothing has touched. See §10.1.

66. **A good day is silent, and a new order arrives a beat late.** Two changes to the same moment -- the rollover -- and they are one idea: movement should be spent on what the player must act on, and nothing else.
    **HOP and NUDGE are retired.** Items 58 and 59 gave the two *paying* tiers a vertical bounce and a shallow lift, on the reasoning that the axis carries the meaning. The reasoning held; the arithmetic did not. A working network pays most of its lines most days, so between them the two covered nearly every settlement on nearly every rollover -- and motion that fires when **nothing is wrong** cannot mean "look here". It is item 30's objection reached from the other side: that argument refused a pulsing red halo because on day one *everything* is red, and this is the same failure late in a run, where everything is green. What they said is not lost, and was never only theirs to say: the chip's colour is the payment tier (item 49), and the payout label floats the actual figure at the instant it changes (item 60). What is left is one axis per meaning rather than an axis per tier -- **horizontal is "this did not pay", scale is "this is new"** -- and they can no longer read as amplitudes of one motion, because there is no longer a scale of anything. Red still shakes at day end and still nags every 20s (item 61); that is the whole vocabulary now.
    **A new chip is held back for 0.55 real seconds.** Filling an order opens the next one immediately (DEV-01), so the new chip was drawn by the very render that reported the delivery which earned it. Two things went wrong at once. The verdict and the reward arrived as one indistinguishable frame; and the new line, being undelivered and therefore **red**, sorted straight to the **top** of the column (`_worse_first`) — above the lines that had actually resolved, and, before this item, making a settlement *shake* on the day it had just been paid.
    **The fix is to defer the chip, not to animate it late.** A pop on a chip that is already present does not stop it sorting to the top of a column the player is mid-read. So orders opened by the day are held in `_deferred_orders` and **filtered out of the column entirely**, then released on a render of their own where they pop. Strictly a display hold: `active_orders` opened the line at day end and the simulation has counted it from that moment, because deferring the **order** rather than the chip would change what the next day delivers. Timed in **real seconds, unscaled by `DAY_SPEEDS`**, for item 61's reason -- a beat in how the map is read, not an event in the simulation -- and armed on a `SceneTree` timer rather than the day clock, which stops when the player pauses or opens the report and would strand a chip offscreen behind a modal. One timer per day however many orders open.
