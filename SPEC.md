...
- **Shadow resolution is sized for a phone.** The map is a 2048 directional shadow map with the range kept just wide enough to cover the whole region from any in-game zoom or pan (70 units) -- a wider range spreads the same map over more ground, and the grass block's corner tufts start aliasing into a diagonal hatch. 4096 was tried first and dropped: it cost four times the shadow memory and was enough to get the WebGL context evicted on a phone browser, and its only visible benefit was at magnifications past what the game's own zoom allows.
